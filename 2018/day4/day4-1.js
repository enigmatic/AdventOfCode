...
for (i = 0; i < parsedData.length; i++) {
  console.log(parsedData[i].original);
  if (parsedData[i].asleep) {
    ...
    console.log(sleepTime.guard + ' slept for ' + sleepTime.length);
  }
}
sleepTime = {};
sleepHours = {};
...
var restedGuard = -1;
var maxSleep = 0;
for (guardID in sleepTime) {
  if (sleepTime[guardID] > maxSleep) {
    maxSleep = sleepTime[guardID];
    restedGuard = guardID;
  } 
}
console.log('Guard : ' + restedGuard); 

var bestMinute = 0;
var maxDays = 0
var guardSleepSchedule = sleepHours[restedGuard]
...
console.log("Hash:" + (bestMinute * restedGuard));